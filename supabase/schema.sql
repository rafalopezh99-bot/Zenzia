...
create table packages (
  ...
  contact_id uuid not null references contacts(id) on delete cascade,
  name text not null,
  -- numeric (no int): permite bonos con horas fraccionarias (ej. clases de
  -- 1h30 restan 1.5), consumidas automáticamente por
  -- complete_finished_academia_appointments() más abajo.
  total_sessions numeric(6,2) not null,
  used_sessions numeric(6,2) not null default 0,
  created_at timestamptz not null default now()
);
...
-- El cron diario no llama a run_billing_cycle() directamente: llama a
-- run_daily_maintenance() (ver más abajo), que a su vez llama a esta y a
-- generate_recurring_appointments().

-- ============================================================
-- HORARIOS RECURRENTES DE CLASE (vertical academia)
-- ============================================================
-- Para no dar de alta cada clase a mano: un horario define un alumno + día
-- de la semana + franja horaria, y se materializa como "appointments" de
-- verdad en una ventana móvil de 8 semanas — mismo patrón que el cobro
-- recurrente de bonos de arriba. Al crear el horario se genera ya la
-- primera tanda (ver lib/actions/appointments.ts, createClassSchedule); el
-- ciclo diario se encarga de ir rellenando las semanas siguientes.
...
-- ciclo diario vuelve a pasar por la misma semana. Se añade aquí (y no en
-- el create table appointments de arriba) porque class_schedules todavía
-- no existía en ese punto del archivo.
...
-- Barre TODAS las empresas (no solo la del caller: es security definer y se
-- llama tanto desde el cron como, para frescura casi en tiempo real, desde
-- varias páginas del panel cuando el vertical es academia — ver
-- app/(dashboard)/{dashboard,citas,citas/calendario,seguimiento}/page.tsx).
-- Por cada cita de academia ya terminada (ends_at < now(), sigue "scheduled"):
-- la marca "completed" y descuenta sus horas (duración real, con decimales)
-- del bono activo más reciente del alumno que tenga bono_type_id (es decir,
-- del tarifario, no un paquete manual/ad-hoc, que no se consume solo).
...
-- proyecto): select cron.schedule('zenzia-daily-maintenance', '0 5 * * *',
...
-- ============================================================
-- MI WEB: contenido autogestionable de la web del cliente
-- ============================================================
-- Módulo "sitio_web": permite que una empresa autogestione el contenido de
-- su propia web (plantilla ya entregada por RL Digital Studios) sin tocar
-- el CRM. Pensado para venderse suelto, sin el resto de módulos — por eso
-- NO se añade a ningún VERTICAL_PACKS por defecto (ver lib/modules.ts): se
-- activa a mano, empresa por empresa, igual que cualquier otro módulo.
-- Ver migración 2026-09-14-site-content-module.sql.
...
-- (mismo criterio que "public landing notification form" de arriba, que
-- también abre una rendija puntual para el rol anon).
